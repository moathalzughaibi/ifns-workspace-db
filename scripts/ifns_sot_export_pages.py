...
def safe_filename(title):
    s = title
    for bad, rep in SAFE_CHAR_MAP.items():
        s = s.replace(bad, rep)
    s = s.replace("—", "-").replace("–", "-")
...
def to_md(blocks):
    def rt(x):
        return "".join([t.get("plain_text", "") for t in x]) if x else ""

    lines = []
    for b in blocks:
        t = b.get("type")
        if t == "heading_1":
            lines.append("# " + rt(b["heading_1"]["rich_text"]))
        elif t == "heading_2":
            lines.append("## " + rt(b["heading_2"]["rich_text"]))
        elif t == "paragraph":
            lines.append(rt(b["paragraph"]["rich_text"]))
        elif t == "bulleted_list_item":
            lines.append("- " + rt(b["bulleted_list_item"]["rich_text"]))
        elif t == "numbered_list_item":
            lines.append("1. " + rt(b["numbered_list_item"]["rich_text"]))
        elif t == "callout":
            lines.append("> " + rt(b["callout"]["rich_text"]))
        elif t == "divider":
            lines.append("\n---\n")
    return "\n".join(lines).strip() + "\n"
...
def export_children_pages(c, parent_id, out_dir):
    ...
    for k in kids:
        ...
        (pathlib.Path(out_dir) / fname).write_text(md, encoding=ENC)
...
def main():
    ...
    for k in kids:
        if k.get("type") == "child_page":
            ids[k["child_page"]["title"]] = k["id"]
    if "SoT Docs" in ids:
        export_children_pages(c, ids["SoT Docs"], OUT_SOT)
    if "14-Step Spine" in ids:
        export_children_pages(c, ids["14-Step Spine"], OUT_STEPS)
