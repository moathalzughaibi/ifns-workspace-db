...
COMPANIONS = {
    ...
    "Decisions": {
        "properties": {
            ...
            "Impact": {
                "select": {
                    "options": [{"name": "Low"}, {"name": "Med"}, {"name": "High"}]
                }
            },
            "Notes": {"rich_text": {}},
        }
...
def find_or_create_hub_page(notion, db_id):
    ...
    try:
        if hasattr(notion.databases, "query"):
            q = notion.databases.query(
                database_id=db_id,
                filter={"property": tkey, "title": {"equals": HUB_ROW_TITLE}},
            )
            if q.get("results"):
    ...
    row = notion.pages.create(
        ...
        properties={
            tkey: {"title": [{"type": "text", "text": {"content": HUB_ROW_TITLE}}]}
        },
    )
    return row["id"]
...
def add_relations(notion, ids):
    try:
        notion.databases.update(
            ...
            properties={"Project": {"relation": {"database_id": ids["Projects"]}}},
        )
        notion.databases.update(
            database_id=ids["Projects"],
            properties={"Tasks": {"relation": {"database_id": ids["Tasks"]}}},
        )
    except Exception:
        pass
...
def main():
    ...
    ids = create_companions(notion, hub)
    add_relations(notion, ids)
    os.makedirs("IFNS_Workspace_DB/config", exist_ok=True)
    with open(
        "IFNS_Workspace_DB/config/workspace_companion_map.json", "w", encoding=ENC
    ) as f:
