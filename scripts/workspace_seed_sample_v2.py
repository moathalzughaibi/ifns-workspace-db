...
def find_status_key(props):
    ...
    if "Status" in props: return "Status"
    for k,p in props.items():
        if p.get("type") in ("status","select"): return k
    return None
...
def main():
    ...
    # Introspect each DB
    proj_db = c.databases.retrieve(P); proj_props = proj_db.get("properties",{})
    task_db = c.databases.retrieve(T); task_props = task_db.get("properties",{})
    dec_db  = c.databases.retrieve(D); dec_props  = dec_db.get("properties",{})
    appr_db = c.databases.retrieve(A); appr_props = appr_db.get("properties",{})
    hand_db = c.databases.retrieve(H); hand_props = hand_db.get("properties",{})
    ...
    dec_status = find_status_key(dec_props)  # may be None; not required

    appr_title = find_title_key(appr_props)
    appr_status = find_status_key(appr_props)  # not required
    ...
    if proj_status: p_props[proj_status] = {"select":{"name":"Active"}}
    ...
    if task_status: t_props[task_status] = {"select":{"name":"Doing"}}
    if task_proj_rel: t_props[task_proj_rel] = {"relation":[{"id": pid}]}
    ...
    if hand_status: h_props[hand_status] = {"select":{"name":"Draft"}}
