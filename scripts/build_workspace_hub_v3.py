...
def apiprint(e: Exception):
    if isinstance(e, errors.APIResponseError):
        print("APIResponseError:", e.code, getattr(e, "message", ""), getattr(e, "body", ""), file=sys.stderr)
    else:
        print("Exception:", repr(e), file=sys.stderr)

# props helpers
def t(n):return {n:{"title":{}}}
def rt(n):return {n:{"rich_text":{}}}
def sel(n,ops):return {n:{"select":{"options":[{"name":o} for o in ops]}}}
def msel(n,ops):return {n:{"multi_select":{"options":[{"name":o} for o in ops]}}}
def dt(n):return {n:{"date":{}}}
def urlp(n):return {n:{"url":{}}}
def ppl(n):return {n:{"people":{}}}
def rel(n,db):return {n:{"relation":{"database_id":db}}}
...
def ensure_hub(c, hub_id=None):
    ...
    p=c.pages.create(parent={"type":"workspace","workspace":True},
                     properties={"title":{"title":[{"type":"text","text":{"content":"IFNS_Workspace_DB - Hub"}}]}})
    return p["id"]
...
def create_db_under(c, parent_page_id, title, props):
    ...
    try:
        d=c.databases.create(parent={"type":"page_id","page_id":parent_page_id},
...
def seed_admin(c, dbid):
    rows=[("Auto-refresh enabled","mirror.auto_refresh.enabled","true","bool",["mirror"],"UI auto refresh"),
    ...
    for name,key,val,typ,dom,notes in rows:
        c.pages.create(parent={"type":"database_id","database_id":dbid},
          properties={
...
def main():
    ...
    if not token or not root_db:
        print("Missing NOTION_TOKEN or WORKSPACE_DB_ID", file=sys.stderr); sys.exit(2)
    c=Client(auth=token)
    m=load_map()
    ...
    admin_props={**t("Name"),**rt("Key"),**rt("Value"),**sel("Type",["bool","int","float","string"]),
                 **msel("Domain",["mirror","harness","reports"]),**rt("Notes")}
    admin_id, err = create_db_under(c, host, "Admin - Config Index (SoT)", admin_props)
    if err:
        apiprint(err)
        print("Retrying Admin DB under Hub ...", file=sys.stderr)
        admin_id, err = create_db_under(c, hub, "Admin - Config Index (SoT)", admin_props)
        if err:
            apiprint(err); sys.exit(3)
    ...
    def mk(title, props):
        ...
        if e:
            ...
            if e: apiprint(e); sys.exit(4)
        return dbid

    projects_id = mk("Projects (SoT)", {**t("Name"),**sel("Status",["Planned","Active","On hold","Done"]),
                        **sel("Priority",["High","Medium","Low"]),**ppl("Owner"),**dt("Start"),**dt("End"),
                        **msel("Tags",["SoT","UX","Runtime","Telemetry","DB"]),**rt("Description"),
                        **rel("Workspace", root_db)})

    tasks_id = mk("Tasks (SoT)", {**t("Name"),**sel("Status",["Not Started","In Progress","Blocked","Done"]),
                        **sel("Priority",["High","Medium","Low"]),**ppl("Assignee"),**dt("Due"),
                        **rel("Project", projects_id),**rel("Workspace", root_db),
                        **msel("Tags",["SoT","UX","Runtime","Telemetry","DB"])})

    decisions_id = mk("Decisions (SoT)", {**t("Decision"),**sel("Status",["Proposed","Approved","Rejected","Changed"]),
                        **dt("Date"),**ppl("Owner"),**rel("Project", projects_id),
                        **rel("Workspace", root_db),**rt("Notes")})

    approvals_id = mk("Approvals (SoT)", {**t("Request"),**sel("Status",["Pending","Approved","Denied"]),
                        **dt("Date"),**rt("RequestedBy"),**rt("ApprovedBy"),
                        **rel("Project", projects_id),**rel("Decision", decisions_id),
                        **rel("Workspace", root_db)})

    handover_id = mk("Handover (SoT)", {**t("Item"),**sel("Area",["Docs","ETL","Runtime","UX/SxE","QC/Telemetry"]),
                        **sel("Status",["Pending","Ready","Delivered"]),**dt("Due"),
                        **ppl("Owner"),**urlp("Link"),**rel("Project", projects_id),
                        **rel("Workspace", root_db)})

    # 5) Save map and print
    m.update({"db_host_page_id":host,"admin_config_db_id":admin_id,"projects_db_id":projects_id,
              "tasks_db_id":tasks_id,"decisions_db_id":decisions_id,"approvals_db_id":approvals_id,
