...
def flat(page):
    ...
    for k,p in page["properties"].items():
        t=p.get("type")
        if t=="title": out[k]="".join(x["plain_text"] for x in p["title"]) if p["title"] else ""
        elif t=="rich_text": out[k]="".join(x["plain_text"] for x in p["rich_text"]) if p["rich_text"] else ""
        elif t=="select": out[k]=p["select"]["name"] if p["select"] else ""
        elif t=="status": out[k]=p["status"]["name"] if p["status"] else ""
        elif t=="multi_select": out[k]=";".join(o["name"] for o in p["multi_select"])
        elif t=="people": out[k]=";".join(peep["name"] for peep in p["people"])
        elif t=="date": out[k]=p["date"]["start"] if p["date"] else ""
        elif t=="checkbox": out[k]="true" if p["checkbox"] else "false"
        elif t=="url": out[k]=p["url"] or ""
        else: out[k]=""
    return out
