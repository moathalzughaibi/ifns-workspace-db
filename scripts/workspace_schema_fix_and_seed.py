...
def ensure_status_like(notion, dbid):
    ...
    try:
        notion.databases.update(
            ...
            properties={ "Status": { "status": { "options": [
                {"name":"Planned"},{"name":"In Review"},{"name":"Approved"},{"name":"Done"}
            ]}}}
        )
        return ("Status","status")
    ...
    notion.databases.update(
        database_id=dbid,
        properties={ "State": { "select": { "options": [
            {"name":"Planned"},{"name":"In Review"},{"name":"Approved"},{"name":"Done"}
        ]}}}
    )
    return ("State","select")
...
def upsert_seed(notion, dbid, title_prop, status_prop_name):
    ...
    if cur.get("results"): return
    today = datetime.date.today().isoformat()
    samples = [
        {
            title_prop: {"title":[{"type":"text","text":{"content":"Workspace bootstrap"}}]},
            status_prop_name: {"status":{"name":"In Review"}} if status_prop_name=="Status"
        ...
        {
            title_prop: {"title":[{"type":"text","text":{"content":"Add Projects/Tasks DBs"}}]},
            status_prop_name: {"status":{"name":"Planned"}} if status_prop_name=="Status"
